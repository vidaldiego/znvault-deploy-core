// Path: src/types.ts
// Deploy-core type definitions

import type { Command } from 'commander';

/**
 * Generic outcome of a single deploy operation against one host.
 *
 * This is the target-agnostic shape every deployer plugin's own
 * (possibly richer) deploy-result type must be assignable to/from when
 * feeding results into the shared strategy-executor / multi-class-deploy
 * machinery. Deployer plugins are free to carry additional fields on their
 * own concrete result types.
 */
...
/**
 * Context accumulated while running a (possibly multi-batch) deploy against
 * a set of hosts. Consumed structurally by `multi-class-deploy.ts`'s
 * blocking gate (`classGateFailed`) and summary printer — callers that
 * build a richer, orchestration-specific context (e.g. a Listr2 task
 * context) only need to be assignable to this shape.
 */
...
/**
 * Narrow structural contract `strategy-executor.ts` needs from a progress
 * reporter — just enough to show canary batch headers/results without
 * depending on any concrete reporter implementation (e.g. the CLI's
 * terminal-rendering `ProgressReporter`). Any object exposing these three
 * methods satisfies `StrategyExecutorOptions.progress`.
 */
export interface StrategyProgressReporter {
  showBatchHeader(batchNum: number, totalBatches: number, hostCount: number, label: string): void;
  showBatchResult(batchNum: number, successes: number, failures: number): void;
  showCanaryAbort(failedBatch: number, skippedCount: number): void;
}
